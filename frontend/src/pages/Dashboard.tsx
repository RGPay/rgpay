...
const Dashboard: React.FC = () => {
  ...
  const getFormaPagamentoChartOptions = () => {
    ...
    const formas = metrics.faturamentoPorFormaPagamento.map((item) => item.forma_pagamento);
    const valores = metrics.faturamentoPorFormaPagamento.map((item) => item.total);
    const colorPalette = [
      theme.palette.primary.main,
      theme.palette.secondary.main,
      theme.palette.success.main,
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <GradientCard
            ...
            secondaryValue={`${metrics.produtosMaisVendidos?.length || 0} produtos diferentes`}
            change={-3.5}
